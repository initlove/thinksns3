...
class IndexAction extends Action {
     ...
     */
        ...
        function add_pic() {
            ...
            try{
                $result = $voteDao->addVotePic($data,$opt,$attachid);
            }catch(ThinkException $e){
                $this->error($e->getMessage());
            }
            ...
            if($result){
                ...
                $this->ajaxData['title'] = keyWordFilter($data['title']);
                $this->ajaxData['opt']   = array_filter(keyWordFilter($opt));
                $this->ajaxData['attachid']   = array_filter(keyWordFilter($attachid));
                $this->ajaxData['deadline'] = $data['deadline'];
                // $this->success('添加投票成功');
                $this->ajaxReturn($this->ajaxData, '添加投票成功', 1);
            }else{
     ...
     * 某个投票的详情
      */
        function pollDetail() {
                ...
                foreach($vote_opts as $key=>$v){
                    $attachInfo = model('Attach')->getAttachByIds($v['attachid']);
                    $vote_opts[$key]['src'] = getImageUrl($attachInfo[0]['save_path'].$attachInfo[0]['save_name'],180,180,true);
                    $vote_opts[$key]['src-big'] = getImageUrl($attachInfo[0]['save_path'].$attachInfo[0]['save_name']);
                }
     ...
     * 投票
      */
        ...
        function addOptPic() {
                ...
                $map['name'] = t(h($_POST['name']));
                //增加图片投票
                $map['attachid']=t(h($_POST['attachid']));
                $map['vote_id'] = $id;
                //查找这个投票的所有选项
                $voteDao = D( 'VoteOpt' );
                $vote_opt = $voteDao->where( "vote_id = {$id}" )->field()->findAll();
